document.addEventListener("DOMContentLoaded", () => {
  ...
  folderInput.addEventListener("change", (event) => {
    ...
    htmlFiles = Array.from(files).filter((file) =>
  ...
  processButton.addEventListener("click", async () => {
    ...
    resultsBody.innerHTML = "";
    resultsArea.style.display = "none";
    downloadLink.style.display = "none"; // Ensure download link is hidden too

    processButton.disabled = true;
    status.textContent =
      "Processing... (this may take a moment for many files)";
    downloadLink.style.display = "none";
    ...
    for (const file of htmlFiles) {
      status.textContent = `Processing: ${file.name}`; // Update status per file
      try {
        ...
        // *** NEW: 2. Create and append table row ***
        const tr = document.createElement("tr");
        header.forEach((field) => {
          // Iterate using header order
          const td = document.createElement("td");
          td.textContent = parsedRow[field] || ""; // Get data from parsedRow object
          tr.appendChild(td);
        });
        resultsBody.appendChild(tr); // Add row to table body
      } catch (error) {
        console.error(`Error processing file ${file.name}:`, error);
        status.textContent = `Error processing file ${file.name}. Check console for details.`;
        // Optionally add an error row to the table? Or just log it.
        const tr = document.createElement("tr");
        const td = document.createElement("td");
        td.colSpan = header.length; // Make cell span all columns
        td.textContent = `Error processing file: ${file.name}`;
        td.style.color = "red";
        tr.appendChild(td);
    ...
    if (csvData.length > 1) {
      ...
      downloadLink.style.display = "inline-block"; // Show download button *after* table
    } else {
      status.textContent =
        "No data could be extracted from the selected files.";
      resultsArea.style.display = "none";
    }
  });
      ...
      try {
        ...
        if (!isNaN(dt)) {
          ...
          try {
            ...
            fields["Date (Pacific)"] = dt.toLocaleDateString(
              "en-US",
              dateOptions
            ); // e.g., "04/23/2025"
            ...
            fields["Time (Pacific)"] = dt.toLocaleTimeString(
              "en-US",
              timeOptions
            ); // e.g., "08:47:50 AM"
          } catch (tzError) {
            console.error(
              `Error converting timestamp to Pacific Date/Time for ${filename}:`,
              tzError
            );
          ...
          console.warn(
            `Could not parse timestamp: ${tsStrRaw} in file ${filename}`
          );
        ...
        console.warn(
          `Error creating Date object for timestamp: ${tsStrRaw} in file ${filename}`,
          e
        );
        fields["Timestamp (UTC)"] = tsStrRaw;
        fields["Date (Pacific)"] = "Error";
    ...
    if (!fields["Duration"]) {
      ...
      for (const th of allThElements) {
        ...
        if (valueTd && valueTd.tagName === "TD") {
          ...
          if (!fields["Duration"] && key === "Duration")
            fields["Duration"] = value;
        ...
        if (fields["Duration"] && fields["Labels"] && fields["User Deleted"])
          break;
      }
    }
